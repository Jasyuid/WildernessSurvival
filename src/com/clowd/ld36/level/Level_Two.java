...
public class Level_Two extends Level{
	...
	public Level_Two(String path, StateManager manager){
		super(path, manager);
	}
	...
	public void update2(Input input){
		...
		if(eSpawn){
			...
			if(section == 0){
				...
				if(chance < 35){
					addEntity(new Squirel(this, 13*32, r.nextInt(14*32 - 9*32) + 9*32, 0));
				}else if(chance < 70){
					addEntity(new Wolf(this, 13*32, r.nextInt(14*32 - 9*32) + 9*32));
				}else if(chance < 90){
					addEntity(new WhiteWolf(this, 13*32, r.nextInt(14*32 - 9*32) + 9*32));
				}else if(chance < 100){
					addEntity(new EvilMan(this, 13*32, r.nextInt(13*32 - 9*32) + 9*32, 0));
				}
			}else if(section == 1){
				int chance = r.nextInt(100);
				if(chance < 35){
					addEntity(new Squirel(this, 58*32, r.nextInt(14*32 - 9*32) + 9*32, 1));
				}else if(chance < 70){
					addEntity(new Wolf(this, 58*32, r.nextInt(14*32 - 9*32) + 9*32));
				}else if(chance < 90){
					addEntity(new WhiteWolf(this, 58*32, r.nextInt(14*32 - 9*32) + 9*32));
				}else if(chance < 100){
					addEntity(new EvilMan(this, 58*32, r.nextInt(13*32 - 9*32) + 9*32, 1));
				}
			}
